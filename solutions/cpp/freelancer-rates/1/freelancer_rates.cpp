...
int monthly_rate(double hourly_rate, double discount) {
    ...
    return static_cast<int>(std::ceil(apply_discount(monthly_rate, discount)));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double days_to_budget = daily_rate(hourly_rate);
    double days_budget = budget / (days_to_budget * ((100 - discount) / 100));
    
    return static_cast<int>(std::floor(days_budget));
}
